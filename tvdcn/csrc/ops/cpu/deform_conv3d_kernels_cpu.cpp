...
namespace tvdcn {
    namespace ops {
        namespace {
            ...
            template<typename scalar_t>
            static __forceinline__ scalar_t trilinear_coordinate_weight(
                    const scalar_t *input,
                    const int depth,
                    const int height,
                    const int width,
                ...
                for (int i = 0; i < weight_d; ++i) {
                    for (int j = 0; j < weight_h; ++j) {
                        for (int k = 0; k < weight_w; ++k) {
                            ...
                            const int z = (out_z * stride_d - pad_d) + i * dilation_d;
                            const int y = (out_y * stride_h - pad_h) + j * dilation_h;
                            const int x = (out_x * stride_w - pad_w) + k * dilation_w;
                            ...
                            const scalar_t val =
                                    ...
                                    interpolate_sample(
                                            ...
                                            y +
                                            offset_ptr[(((offset_idx + 1) * out_d + out_z) * out_h + out_y) * out_w +
                                                       out_x],
                                            x +
                                            offset_ptr[(((offset_idx + 2) * out_d + out_z) * out_h + out_y) * out_w +
                                                       out_x])
                                               : sample(input_ptr, depth, height, width, z, y, x);
        ...
        static void col2vol_kernel(
            ...
            CPU_1D_KERNEL_LOOP(index, n_kernels) {
                ...
                if (deformable)
                    interpolate_insert(grad_input_ptr, depth, height, width,
                                       z +
                                       offset_ptr[((offset_idx * out_d + out_z) * out_h + out_y) * out_w + out_x],
                                       y +
            ...
            CPU_1D_KERNEL_LOOP(index, n_offset_kernels) {
                ...
                for (int col_c = (offset_c / 3); col_c < c_bound; col_c += col_step) {
                    ...
                    const scalar_t weight =
                            trilinear_coordinate_weight(
                                    input_ptr, depth, height, width,
                                    z + offset_ptr[((offset_idx * out_d + d) * out_h + h) * out_w + w],
                                    y + offset_ptr[(((offset_idx + 1) * out_d + d) * out_h + h) * out_w + w],
                                    x + offset_ptr[(((offset_idx + 2) * out_d + d) * out_h + h) * out_w + w],
                                    direction);

                    const scalar_t mask_val =
                            modulated ?
                            mask_ptr[((mask_idx * out_d + d) * out_h + h) * out_w + w]
                                      : static_cast<scalar_t>(1);
        ...
        static void deform_conv3d_compute_grad_mask_kernel(
            ...
            CPU_1D_KERNEL_LOOP(index, n_mask_kernels) {
                ...
                auto grad_mask_ptr = grad_mask +
                                     (b * n_mask_grps + mask_grp) * weight_d * weight_h * weight_w * out_d * out_h *
                                     out_w;
