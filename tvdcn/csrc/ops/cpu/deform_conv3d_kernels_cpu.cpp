...
static scalar_t interpolate_sample(
    ...
    val += (valid_z_l && valid_y_l && valid_x_l) ?
           dz_l * dy_l * dx_l * input[z_l * (width * height) + y_l * width + x_l] : static_cast<scalar_t>(0);
    val += (valid_z_l && valid_y_l && valid_x_h) ?
           dz_l * dy_l * dx_h * input[z_l * (width * height) + y_l * width + x_h] : static_cast<scalar_t>(0);
    val += (valid_z_l && valid_y_h && valid_x_l) ?
           dz_l * dy_h * dx_l * input[z_l * (width * height) + y_h * width + x_l] : static_cast<scalar_t>(0);
    val += (valid_z_l && valid_y_h && valid_x_h) ?
           dz_l * dy_h * dx_h * input[z_l * (width * height) + y_h * width + x_h] : static_cast<scalar_t>(0);
    val += (valid_z_h && valid_y_l && valid_x_l) ?
           dz_h * dy_l * dx_l * input[z_h * (width * height) + y_l * width + x_l] : static_cast<scalar_t>(0);
    val += (valid_z_h && valid_y_l && valid_x_h) ?
           dz_h * dy_l * dx_h * input[z_h * (width * height) + y_l * width + x_h] : static_cast<scalar_t>(0);
    val += (valid_z_h && valid_y_h && valid_x_l) ?
           dz_h * dy_h * dx_l * input[z_h * (width * height) + y_h * width + x_l] : static_cast<scalar_t>(0);
    val += (valid_z_h && valid_y_h && valid_x_h) ?
           dz_h * dy_h * dx_h * input[z_h * (width * height) + y_h * width + x_h] : static_cast<scalar_t>(0);
...
static inline void insert(
    ...
    if (0 <= z && z < depth && 0 <= y && y < height && 0 <= x && x < width)
        output[z * height * width + y * width + x] += val;
}
...
static void interpolate_insert(
    ...
    if (valid_z_l && valid_y_l && valid_x_l)
        output[z_l * height * width + y_l * width + x_l] += dz_l * dy_l * dx_l * val;
    if (valid_z_l && valid_y_l && valid_x_h)
        output[z_l * height * width + y_l * width + x_h] += dz_l * dy_l * dx_h * val;
    if (valid_z_l && valid_y_h && valid_x_l)
        output[z_l * height * width + y_h * width + x_l] += dz_l * dy_h * dx_l * val;
    if (valid_z_l && valid_y_h && valid_x_h)
        output[z_l * height * width + y_h * width + x_h] += dz_l * dy_h * dx_h * val;
    if (valid_z_h && valid_y_l && valid_x_l)
        output[z_h * height * width + y_l * width + x_l] += dz_h * dy_l * dx_l * val;
    if (valid_z_h && valid_y_l && valid_x_h)
        output[z_h * height * width + y_l * width + x_h] += dz_h * dy_l * dx_h * val;
    if (valid_z_h && valid_y_h && valid_x_l)
        output[z_h * height * width + y_h * width + x_l] += dz_h * dy_h * dx_l * val;
    if (valid_z_h && valid_y_h && valid_x_h)
        output[z_h * height * width + y_h * width + x_h] += dz_h * dy_h * dx_h * val;
}

template<typename scalar_t>
static scalar_t trilinear_coordinate_weight(
        const scalar_t *input,
        const int depth,
        const int height,
        const int width,
...
static void vol2col_kernel(
    ...
    CPU_1D_KERNEL_LOOP(index, n_kernels) {
        ...
        int offset_ptr = (out_b * n_offset_grps + offset_grp) * 3 * weight_d * weight_h * weight_w * out_d * out_h *
                         out_w;
        int mask_ptr = (out_b * n_mask_grps + mask_grp) * weight_d * weight_h * weight_w * out_d * out_h * out_w;
            ...
            for (int j = 0; j < weight_h; ++j) {
                for (int k = 0; k < weight_w; ++k) {
                    ...
                    const scalar_t offset_h = deformable ?
                                              offset[offset_ptr + (offset_idx + 1) * (out_d * out_h * out_w) +
                                                     out_z * (out_h * out_w) +
                                                     out_y * out_w + out_x] : static_cast<scalar_t>(0);
                    const scalar_t offset_w = deformable ?
                                              offset[offset_ptr + (offset_idx + 2) * (out_d * out_h * out_w) +
                                                     out_z * (out_h * out_w) +
                                                     out_y * out_w + out_x] : static_cast<scalar_t>(0);
...
static void col2vol_kernel(
    ...
    CPU_1D_KERNEL_LOOP(index, n_kernels) {
        ...
        int offset_ptr = (b * n_offset_grps + offset_grp) * 3 * weight_d * weight_h * weight_w * out_d * out_h * out_w;
        int mask_ptr = (b * n_mask_grps + mask_grp) * weight_d * weight_h * weight_w * out_d * out_h * out_w;
        ...
        const scalar_t mask_val = modulated ?
                                  mask[mask_ptr + ((mask_idx * out_d + out_z) * out_h + out_y) * out_w + out_x]
...
static void deform_conv3d_compute_grad_offset_kernel(
    ...
    CPU_1D_KERNEL_LOOP(index, n_offset_kernels) {
        ...
        int col_ptr = offset_grp * c_per_offset_grp * weight_d * weight_h * weight_w * batch_sz * out_d * out_h *
                      out_w;
        ...
        for (int col_c = (offset_c / 3); col_c < c_bound; col_c += col_step) {
            ...
            int mask_idx = i * weight_h * weight_w + j * weight_w + k;
            int offset_idx = 3 * mask_idx;
            const scalar_t offset_d = offset[offset_ptr + ((offset_idx * out_d + out_z) * out_h + out_y) * out_w +
                                             out_x];
            const scalar_t offset_h = offset[offset_ptr + (((offset_idx + 1) * out_d + out_z) * out_h + out_y) * out_w +
                                             out_x];
            const scalar_t offset_w = offset[offset_ptr + (((offset_idx + 2) * out_d + out_z) * out_h + out_y) * out_w +
                                             out_x];
            ...
            const scalar_t mask_val = modulated ?
                                      mask[mask_ptr + mask_idx * (out_d * out_h * out_w) + out_z * (out_h * out_w) +
                                           out_y * out_w +
                                           out_x] : static_cast<scalar_t>(1);

            const scalar_t weight =
                    trilinear_coordinate_weight(input + input_ptr, depth, height, width, z, y, x, direction);
...
void deform_conv3d_compute_grad_offset_cpu(
    ...
    const int n_offset_kernels = out_d * out_h * out_w * 3 * weight_d * weight_h * weight_w * n_offset_grps * batch_sz;
...
static void deform_conv3d_compute_grad_mask_kernel(
    ...
    CPU_1D_KERNEL_LOOP(index, n_mask_kernels) {
        ...
        int col_ptr = mask_grp * c_per_mask_grp * weight_d * weight_h * weight_w * batch_sz * out_d * out_h * out_w;
        ...
        for (int col_c = mask_c; col_c < c_bound; col_c += col_step) {
            ...
            int offset_idx = 3 * mask_idx;
            const scalar_t offset_d = deformable ?
                                      offset[offset_ptr + ((offset_idx * out_d + out_z) * out_h + out_y) * out_w +
                                             out_x]
                                                 : static_cast<scalar_t>(0);
            const scalar_t offset_h = deformable ?
                                      offset[offset_ptr + (((offset_idx + 1) * out_d + out_z) * out_h + out_y) * out_w +
                                             out_x]
                                                 : static_cast<scalar_t>(0);
            const scalar_t offset_w = deformable ?
                                      offset[offset_ptr + (((offset_idx + 2) * out_d + out_z) * out_h + out_y) * out_w +
                                             out_x]
                                                 : static_cast<scalar_t>(0);
...
void deform_conv3d_compute_grad_mask_cpu(
    ...
    if (!modulated) return;
    const int n_kernels = out_d * out_h * out_w * weight_d * weight_h * weight_w * in_channels * batch_sz;
    const int n_mask_kernels = out_d * out_h * out_w * weight_d * weight_h * weight_w * n_mask_grps * batch_sz;
