...
def relocate_elf_library(patchelf, output_dir, output_library, binary):
    ...
    new_libraries_path = osp.join(output_dir, "tvdcn.libs")
    ...
    subprocess.check_output(
        [patchelf, "--set-rpath", "$ORIGIN:$ORIGIN/../tvdcn.libs", binary_path], cwd=output_library
    )
...
def relocate_dll_library(dumpbin, output_dir, output_library, binary):
    ...
    print("Copying dependencies to wheel directory")
    package_dir = osp.join(output_dir, "tvdcn")
    for library in binary_paths:
        if library != binary:
            library_path = binary_paths[library]
...
def patch_linux():
    ...
    wheels = glob.glob(osp.join(PACKAGE_ROOT, "wheelhouse", "*.whl"))
    output_dir = osp.join(PACKAGE_ROOT, "wheelhouse", ".wheel-process")
    ...
    for wheel in wheels:
        ...
        output_library = osp.join(output_dir, "tvdcn")
